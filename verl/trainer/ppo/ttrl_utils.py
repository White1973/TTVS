...
#         selected_indices_cur=random.sample(range(start, start+n_votes_per_prompt), n_samples_per_prompt)
#         print('***************', start, start+n_votes_per_prompt, len(selected_indices),'***************', selected_indices_cur, '***************')
#         selected_indices.extend(selected_indices_cur)
...
def select_top_k_per_prompt(data, n_votes_per_prompt, n_samples_per_prompt, mixrollouts=False):
    """
    Select the first k rollouts per prompt, used for TTRL downsampling.
    """
    assert len(data) % n_votes_per_prompt == 0, "data length must be divisible by n_votes_per_prompt"
    ...
    for i in range(num_prompts):
        start = i * n_votes_per_prompt
        if mixrollouts:
            selected_indices_ori=random.sample(range(start, start+int(n_votes_per_prompt/2)), int(n_samples_per_prompt/2))
            selected_indices_aug=random.sample(range(start+int(n_votes_per_prompt/2), start + n_votes_per_prompt), int(n_samples_per_prompt/2))  
            selected_indices.extend(selected_indices_ori)
            selected_indices.extend(selected_indices_aug)
        else:
            selected_indices.extend(random.sample(range(start, start+n_votes_per_prompt), n_samples_per_prompt))
            # print('***************', start, start+n_votes_per_prompt, len(selected_indices),'***************', range(start, start + n_samples_per_prompt), '***************')
...
def apply_ttrl_gt(batch, gen_batch_output, n, tokenizer):
    ...
    for i in range(num_prompts):
        ...
        for j in range(n):
            ...
            valid_response_length = data_item.batch["attention_mask"][prompt_length:].sum() #Calculate the sum of attention_mask over the part of the response (after the prompt) to accurately get the effective length of the response
...
def _batch_compute_ttrl_metrics(
    ...
    for i in range(n_prompts):
        ...
        #Critical sanity checks. It asserts that for n samples of the same prompt, their majority_label must be exactly the same. This is because majority_label is the only result that comes from voting on these n samples and is given to each of these samples. prompt_gt_label Same.
